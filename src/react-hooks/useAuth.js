...
export default function useAuth(firebaseAuth) {
    ...
    const createEmailUser = (email, password) => firebaseAuth().createUserWithEmailAndPassword(email, password);

    const signInEmailUser = (email, password) => firebaseAuth().signInEmailUser(email, password);
